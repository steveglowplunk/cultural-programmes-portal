...
db.once("open", function () {
        ...
        res.on("end", async () => {
          ...
          if (venues) {
            venues.forEach((venue: string) => {
              ...
              const venueNameMatch = venue.match(
                /<venuee><!\[CDATA\[(.*?)\]\]><\/venuee>/
              );
              const latitudeMatch = venue.match(
                /<latitude><!\[CDATA\[(.*?)\]\]><\/latitude>/
              );
              const longitudeMatch = venue.match(
          ...
          try {
            ...
            if (events) {
              for (const event of events) {
                ...
                const titleEMatch = event.match(
                  /<titlee><!\[CDATA\[(.*?)\]\]><\/titlee>/
                );
                const cat1Match = event.match(
                  /<cat1><!\[CDATA\[(.*?)\]\]><\/cat1>/
                );
                const cat2Match = event.match(
                  /<cat2><!\[CDATA\[(.*?)\]\]><\/cat2>/
                );
                const predateEMatch = event.match(
                  /<predateE><!\[CDATA\[(.*?)\]\]><\/predateE>/
                );
                const progtimeEMatch = event.match(
                  /<progtimee><!\[CDATA\[(.*?)\]\]><\/progtimee>/
                );
                const venueIdMatch = event.match(
                  /<venueid><!\[CDATA\[(.*?)\]\]><\/venueid>/
                );
                const ageLimitEMatch = event.match(
                  /<agelimite><!\[CDATA\[(.*?)\]\]><\/agelimite>/
                );
                const priceEMatch = event.match(
                  /<pricee><!\[CDATA\[(.*?)\]\]><\/pricee>/
                );
                const descEMatch = event.match(
                  /<desce><!\[CDATA\[(.*?)\]\]><\/desce>/
                );
                const urlEMatch = event.match(
                  /<urle><!\[CDATA\[(.*?)\]\]><\/urle>/
                );
                const tagentUrlEMatch = event.match(
                  /<tagenturle><!\[CDATA\[(.*?)\]\]><\/tagenturle>/
                );
                const remarkEMatch = event.match(
                  /<remarke><!\[CDATA\[(.*?)\]\]><\/remarke>/
                );
                const enquiryMatch = event.match(
                  /<enquiry><!\[CDATA\[(.*?)\]\]><\/enquiry>/
                );
                const saleDateMatch = event.match(
                  /<saledate><!\[CDATA\[(.*?)\]\]><\/saledate>/
                );
                const interBookMatch = event.match(
                  /<interbook><!\[CDATA\[(.*?)\]\]><\/interbook>/
                );
                const presenterOrgEMatch = event.match(
                  /<presenterorge><!\[CDATA\[(.*?)\]\]><\/presenterorge>/
                );
                const progImageMatch = event.match(
                  /<prog_image><!\[CDATA\[(.*?)\]\]><\/prog_image>/
                );
                const detailImage1Match = event.match(
                  /<detail_image1><!\[CDATA\[(.*?)\]\]><\/detail_image1>/
                );
                const detailImage2Match = event.match(
                  /<detail_image2><!\[CDATA\[(.*?)\]\]><\/detail_image2>/
                );
                const detailImage3Match = event.match(
                  /<detail_image3><!\[CDATA\[(.*?)\]\]><\/detail_image3>/
                );
                const detailImage4Match = event.match(
                  /<detail_image4><!\[CDATA\[(.*?)\]\]><\/detail_image4>/
                );
                const detailImage5Match = event.match(
                  /<detail_image5><!\[CDATA\[(.*?)\]\]><\/detail_image5>/
                );
                const videoLinkMatch = event.match(
                  /<video_link><!\[CDATA\[(.*?)\]\]><\/video_link>/
                );
                const video2LinkMatch = event.match(
                  /<video2_link><!\[CDATA\[(.*?)\]\]><\/video2_link>/
                );
                const submitDateMatch = event.match(
                  /<submitdate><!\[CDATA\[(.*?)\]\]><\/submitdate>/
                );
                ...
                const presenterOrgE = presenterOrgEMatch
                  ? presenterOrgEMatch[1]
                  : "";
                const progImage = progImageMatch ? progImageMatch[1] : "";
                const detailImage1 = detailImage1Match
                  ? detailImage1Match[1]
                  : "";
                const detailImage2 = detailImage2Match
                  ? detailImage2Match[1]
                  : "";
                const detailImage3 = detailImage3Match
                  ? detailImage3Match[1]
                  : "";
                const detailImage4 = detailImage4Match
                  ? detailImage4Match[1]
                  : "";
                const detailImage5 = detailImage5Match
                  ? detailImage5Match[1]
                  : "";
  ...
  async function fetchAndStoreCommentData() {
    try {
  ...
  async function filterLocationWithDistance(
    lat: number,
    lon: number,
    maxDistance: number
  ) {
  ...
  async function fetchData() {
    ...
    await fetchAndStoreCommentData();
  }
